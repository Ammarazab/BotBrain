...
def generate_launch_description():
    ...
    b2_video_stream_node = LifecycleNode(
        package='b2_pkg',
    ...
    configure_handler_for_write = RegisterEventHandler(
        OnProcessStart(
            target_action=b2_write_node,
            on_start=[EmitEvent(event=ChangeState(
                lifecycle_node_matcher=matches_action(b2_write_node),
    ...
    activate_handler_for_write = RegisterEventHandler(
        OnStateTransition(
            target_lifecycle_node=b2_write_node,
            goal_state='inactive',
    ...
    configure_handler_for_read = RegisterEventHandler(
        OnProcessStart(
            target_action=b2_read_node,
            on_start=[EmitEvent(event=ChangeState(
                lifecycle_node_matcher=matches_action(b2_read_node),
                transition_id=Transition.TRANSITION_CONFIGURE,
            ))]
        )
    )
    activate_handler_for_read = RegisterEventHandler(
        OnStateTransition(
            target_lifecycle_node=b2_read_node,
            goal_state='inactive',
            entities=[EmitEvent(event=ChangeState(
                lifecycle_node_matcher=matches_action(b2_read_node),
                transition_id=Transition.TRANSITION_ACTIVATE,
    ...
    configure_handler_for_video_stream = RegisterEventHandler(
        OnProcessStart(
            target_action=b2_video_stream_node,
            on_start=[EmitEvent(event=ChangeState(
                lifecycle_node_matcher=matches_action(b2_video_stream_node),
                transition_id=Transition.TRANSITION_CONFIGURE,
            ))]
        )
    )
    activate_handler_for_video_stream = RegisterEventHandler(
        OnStateTransition(
            target_lifecycle_node=b2_video_stream_node,
            goal_state='inactive',
            entities=[EmitEvent(event=ChangeState(
                lifecycle_node_matcher=matches_action(b2_video_stream_node),
                transition_id=Transition.TRANSITION_ACTIVATE,
