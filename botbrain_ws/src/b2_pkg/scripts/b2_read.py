#!/usr/bin/env python3
"""
Hardware read/state node for the Unitree B2 quadruped.

Mirrors go2_pkg/scripts/go2_read.py. The Unitree SDK2 ROS2 bridge
exposes the same DDS topics for B2 as for Go2, so the subscriptions
and the motor index ordering are unchanged.
"""
import rclpy
...
class RobotRead(LifecycleNode):
    ...
    def __init__(self):
        ...
        self.get_logger().info("B2 lifecycle read node created, in 'unconfigured' state.")
    ...
    def low_state_subscriber_callback(self, msg):
        ...
        # Unitree motor indexing is identical between Go2 and B2:
