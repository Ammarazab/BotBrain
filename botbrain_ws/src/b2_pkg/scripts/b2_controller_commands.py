#!/usr/bin/env python3
"""
Game-controller to robot-command translator for the Unitree B2.

Mirrors go2_pkg/scripts/go2_controller_commands.py. Button mapping
is shared across the Unitree quadruped lineup so the same scheme
applies to B2; specialty modes that don't exist on B2 (e.g. some
acrobatic gaits) are kept here for compatibility but will simply
fail silently if rejected by the sport API.
"""
import rclpy
...
class OperationModes(LifecycleNode):
    ...
    def __init__(self):
        ...
        self.get_logger().info("B2 controller commands lifecycle node created, in 'unconfigured' state.")
