...
def select_mode(mode: str) -> str:
    """
    Changes Unitree B2 robot mode. If the response returns failed try again one more time.
    ...
    :param mode: mode of the robot between the following:
        ...
        "hello": Makes the robot say hello. Use this every time someone says hi, hello, greet you or anything related.
        "balance_stand": Makes the robot stand up balancing. It allows for the robot to move. The robot must be standing up for this mode to be called.
...
def light_control(control: bool, brightness: int = 5) -> str:
    """
    Control the head light of the B2 robot on and off with its intensity.
